...
# remove the last column (problem_id) and replace with classe
...
write.csv(training, "p-training3.csv")
write.csv(testing, "p-testing3.csv")
```
...
```{r}
# some of the testing columns are a different data type to the training
# convert from int to num so the prediction can be run
t2 = transform(testing,
